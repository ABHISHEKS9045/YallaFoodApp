...
class _OrdersScreenState extends State<OrdersScreen> {
  ...
  langDetector(String text,String? lang) async {
  ...
  Future<List<int>> getTicket(
    ...
    String? lang;
    langDetector(foodName,lang);
    List<int> arBytes = [];
   // Arabic customer name
    if (lang == "ar") {
       arBytes = utf8.encode(foodName);
    }
    else if(lang == "ar"){
       arBytes = utf8.encode(foodName);
    }
    ...
    // bytes += generator.text('Order Id: $orderId', styles: const PosStyles(align: PosAlign.center));
    bytes += generator.text("Customer: $customerName", styles: const PosStyles(align: PosAlign.center));
    bytes += generator.text('Address: $address', styles: const PosStyles(align: PosAlign.center));

    bytes += generator.hr();

    bytes += generator.text('Item: ${lang == "en" ? foodName : utf8.decode(arBytes)}', styles: const PosStyles(align: PosAlign.center));
    bytes += generator.text('Qty: $qty', styles: const PosStyles(align: PosAlign.center));
  ...
  Future<void> printTicket(
    ...
    if (isConnected == "true") {
      ...
      List<int> bytes = await getTicket(customerName,orderId, address,foodName,qty,total,deliveryCharge,totalAmount);
      log(bytes.toString());
  ...
  Future<void> setConnect(String mac,
      String customerName,
      String orderId,
      String address,
      String foodName,
    ...
    // print("djjd 1");
    try {
      printTicket(customerName,orderId, address,foodName,qty,total,deliveryCharge,totalAmount);
      final String? result = await BluetoothThermalPrinter.connect(mac);
  ...
  Widget buildOrderItem(OrderModel orderModel, int index, OrderModel? prevModel) {
    ...
    debugPrint("$TAG order date milliseconds =====> ${orderModel.createdAt.millisecondsSinceEpoch}");
    debugPrint("$TAG order date =====> ${DateTime.fromMillisecondsSinceEpoch(orderModel.createdAt.millisecondsSinceEpoch)}");
                      ...
                      children: [
                        ...
                        Row(
                          ...
                          children: [
                            ...
                            orderModel.status == ORDER_STATUS_ACCEPTED ?Padding(
                              ...
                              child: ElevatedButton(
                                ...
                                onPressed: () {
                                  ...
                                  OrderProductModel prod = orderModel.orderProduct[index];
                                  VariantInfo variantInfo = prod.variantInfo;

                                  var variant =  variantInfo.variantOptions;

                                  var a =  variant.toString().replaceAll("{", "");
                                  ...
                                  print(
                                      ...
                                      "${orderModel.address.name}\n"
                                          ...
                                          "${prod.price}\n"
                                          "${prod.quantity}\n"
                                          "vvvvv${variantInfo.variantPrice}\n"
                                          "${c}\n"
                                  ...
                                  printTicket(orderModel.address.name,"","${orderModel.address.line1}\n ${orderModel.address.line1} ",prod.name.toString()
                                      ,prod.quantity.toString(),variantInfo.variantPrice.toString(),"",prod.price.toString());
                                },
                                child: Text(
                                  'Print Order'.tr(),
                ...
                orderModel.notes!.isEmpty
                  ...
                  child: Row(
                    ...
                    children: [
                      ...
                      InkWell(
                        onTap: () {
                          showModalBottomSheet(isScrollControlled: true, isDismissible: true, context: context, backgroundColor: Colors.transparent, enableDrag: true, builder: (BuildContext context) => viewNotesheet(orderModel.notes!));
                        },
                        child: Text(
                          "View".tr(),
                    ...
                    children: [
                        ...
                        Expanded(
                          child: OutlinedButton(
                            style: OutlinedButton.styleFrom(
                              ...
                              side: BorderSide(
                                color: getOrderStatusColor(context, orderModel.status),
                              ),
                            ),
                            onPressed: () => null,
                            child: Text(
                      ...
                      Visibility(
                        visible: orderModel.status == ORDER_STATUS_ACCEPTED ||
                            ...
                            orderModel.status == ORDER_STATUS_SHIPPED,
                        child: Container(
                          padding: EdgeInsets.only(left: Lang == "en" ? 16:0,right:Lang == "en" ? 0:14 ),
                          margin: EdgeInsets.only(right: Lang == "en" ? 15:0, left: Lang == "en" ? 0:15),
                          child: InkWell(
                            onTap: () async {
  ...
  Color getOrderStatusColor(BuildContext context, String status) {
    if(status.toLowerCase() == "Order Accepted".toLowerCase().tr()) {
      return Colors.green;
...
class PrintTicketState extends State<PrintTicket> {
  ...
  Future<void> printTicket() async {
    ...
    if (isConnected == "true") {
      ...
      final result = await BluetoothThermalPrinter.writeBytes(bytes);
      if (result == "true") {
        showAlertDialog(context, "Successfully", "Order details print successfully", true);
      }
    } else {
      getBluetooth();
    }
  ...
  Future<List<int>> getTicket() async {
    ...
    bytes += generator.text(widget.orderModel.address.name, styles: const PosStyles(align: PosAlign.center));
    bytes += generator.text('Address: ${widget.orderModel.address.line1}', styles: const PosStyles(align: PosAlign.center));
