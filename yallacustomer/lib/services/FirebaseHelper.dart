...
class FireStoreUtils {
  static String TAG = "FireStoreUtils";
  ...
  Future<List<BannerModel>> getHomeTopBanner() async {
    ...
    QuerySnapshot<Map<String, dynamic>> bannerHomeQuery = await firestore.collection(MENU_ITEM).where("is_publish", isEqualTo: true).where("position", isEqualTo: "top").orderBy("set_order", descending: false).get();
    await Future.forEach(bannerHomeQuery.docs, (QueryDocumentSnapshot<Map<String, dynamic>> document) {
      try {
       //debugPrint("$TAG get Home Top Banner =======> ${document.data()}");
  ...
  Future<List<BannerModel>> getHomeMiddleBanner() async {
    List<BannerModel> bannerHome = [];
    QuerySnapshot<Map<String, dynamic>> bannerHomeQuery = await firestore.collection(MENU_ITEM).where("is_publish", isEqualTo: true).where("position", isEqualTo: "middle").orderBy("set_order", descending: false).get();
    await Future.forEach(bannerHomeQuery.docs, (QueryDocumentSnapshot<Map<String, dynamic>> document) {
      try {
       //debugPrint("$TAG get Home Middle Banner =======> ${document.data()}");
        bannerHome.add(BannerModel.fromJson(document.data()));
      } catch (e) {
       //debugPrint('FireStoreUtils.getCuisines Parse error $e');
      }
  ...
  static Future<User?> getCurrentUser(String uid) async {
   ...
   //debugPrint("$TAG userDocument exists ==========> ${userDocument.exists}");
   //debugPrint("$TAG userDocument data ==========> ${userDocument.data()}");
  ...
  Future<String?> getRestaurantNearBy() async {
    ...
    if (codQuery.data() != null) {
     ...
     //debugPrint("$TAG get Restaurant Near By =========> $radiusValue");
  ...
  Future<List<ProductModel>> getAllTakeAWayProducts() async {
    ...
    QuerySnapshot<Map<String, dynamic>> productsQuery = await firestore.collection(PRODUCTS).where("takeawayOption", isEqualTo: true).where('publish', isEqualTo: true).get();
    await Future.forEach(productsQuery.docs, (QueryDocumentSnapshot<Map<String, dynamic>> document) {
      try {
        products.add(ProductModel.fromJson(document.data()));
      } catch (e) {
  ...
  Future<List<VendorCategoryModel>> getCuisines() async {
    ...
    List<VendorCategoryModel> cuisines = [];
    // QuerySnapshot<Map<String, dynamic>> cuisinesQuery = await firestore.collection(VENDORS_CATEGORIES).where('publish', isEqualTo: true).where("lang",isEqualTo: Lang).get();
  ...
  Future<List<ProductModel>> foodBycategory(String categoryID) async {
    List<ProductModel> foods = [];
    QuerySnapshot<Map<String, dynamic>> cuisinesQuery = await firestore.collection("vendor_products").where('publish', isEqualTo: true,).where("categoryID", isEqualTo: categoryID,).get();
    await Future.forEach(cuisinesQuery.docs, (QueryDocumentSnapshot<Map<String, dynamic>> document) {
      try {
        foods.add(ProductModel.fromJson(document.data()));
      } catch (e) {
       debugPrint('FireStoreUtils.getCuisines Parse error $e');
      }
    });
  ...
  Stream<List<VendorModel>> getVendors1({String? path}) async* {
    ...
    String field = 'g';
   //debugPrint("$TAG ${radiusValue.toString()} == RADIUS get Vendors1");
    ...
    stream.listen((List<DocumentSnapshot> documentList) {
      ...
      documentList.forEach((DocumentSnapshot document) {
       ...
       //debugPrint("$TAG vendors location =========> ${data['g']['id']} id " + point.latitude.toString() + " ||| " + point.longitude.toString() + " === " + data['title']);
       //debugPrint("$TAG vendors length ========> ${vendors.length}");
      });
  ...
  Future<List<VendorModel>> getVendors() async {
    ...
    await Future.forEach(vendorsQuery.docs, (QueryDocumentSnapshot<Map<String, dynamic>> document) {
      try {
       //debugPrint("$TAG get Vendors =========> ${document.data()}");
        vendors.add(VendorModel.fromJson(document.data()));
       //debugPrint("*-*-/*-*-" + document["title"].toString());
      } catch (e) {
       //debugPrint('FireStoreUtils.getVendors Parse error $e');
      }
    });
    return vendors;
  }
    ...
    ordersStreamSub = firestore.collection(ORDERS).where('authorID', isEqualTo: userID).orderBy('createdAt', descending: true).snapshots().listen((onData) async {
      ...
      await Future.forEach(onData.docs, (QueryDocumentSnapshot<Map<String, dynamic>> element) {
        try {
         ...
         //debugPrint('watchOrdersStatus parse error ${element.id} $e $s');
        }
      });
  ...
  Stream<List<BookTableModel>> getBookingOrders(String userID, bool isUpComing) async* {
    ...
    if (isUpComing) {
      ...
      firestore.collection(ORDERS_TABLE).where('author.id', isEqualTo: userID).where('date', isGreaterThan: Timestamp.now()).orderBy('date', descending: true).orderBy('createdAt', descending: true).snapshots().listen((onData) async {
        await Future.forEach(onData.docs, (QueryDocumentSnapshot<Map<String, dynamic>> element) {
          try {
            orders.add(BookTableModel.fromJson(element.data()));
          } catch (e, s) {
           //debugPrint('booktable parse error ${element.id} $e $s');
          }
        });
      ...
      firestore.collection(ORDERS_TABLE).where('author.id', isEqualTo: userID).where('date', isLessThan: Timestamp.now()).orderBy('date', descending: true).orderBy('createdAt', descending: true).snapshots().listen((onData) async {
        await Future.forEach(onData.docs, (QueryDocumentSnapshot<Map<String, dynamic>> element) {
          try {
            orders.add(BookTableModel.fromJson(element.data()));
          } catch (e, s) {
  ...
  Stream<List<VendorModel>> getCategoryRestaurants(String categoryId) async* {
    ...
    try {
      ...
      stream.listen((List<DocumentSnapshot> documentList) {
        ...
        for (var document in documentList) {
          final data = document.data() as Map<String, dynamic>;
         //debugPrint("$TAG get Category Restaurants data ========> $data");
  ...
  Stream<List<VendorModel>> getAllDineInRestaurants() async* {
    ...
    String field = 'g';
   //debugPrint(radiusValue.toString() + "===RADIUSgetVendorsForNewArrival");
    Stream<List<DocumentSnapshot>> stream = geo.collection(collectionRef: collectionReference).within(center: center, radius: radiusValue, field: field, strictMode: true);
    // Stream<List<DocumentSnapshot>> stream = geo.collection(collectionRef: collectionReference).within(center: center, radius: radiusValue, field: field, strictMode: true);
  ...
  Stream<List<VendorModel>> getVendorsByCuisineID(String cuisineID, {bool? isDinein}) async* {
    ...
    List<VendorModel> vendors = [];
    Query<Map<String, dynamic>> collectionReference = isDinein! ? firestore.collection(VENDORS).where('categoryID', isEqualTo: cuisineID).where("enabledDiveInFuture", isEqualTo: true) : firestore.collection(VENDORS).where('categoryID', isEqualTo: cuisineID);
    GeoFirePoint center = geo.point(latitude: MyAppState.selectedPosotion.latitude, longitude: MyAppState.selectedPosotion.longitude);
    String field = 'g';

   //debugPrint("$TAG radiusValue =======> $radiusValue");
   //debugPrint("$TAG field =======> $field");
   //debugPrint("$TAG center latitude =======> ${center.latitude}");
   //debugPrint("$TAG center longitude =======> ${center.longitude}");
   //debugPrint("$TAG collectionReference =======> ${collectionReference.parameters.toString()}");
    ...
    stream.listen((List<DocumentSnapshot> documentList) {
      Future.forEach(documentList, (DocumentSnapshot element) {
       ...
       //debugPrint("$TAG get Vendors By Cuisine ID ========> $data");
        vendors.add(VendorModel.fromJson(data));
  ...
  Future<List<OfferModel>> getAllCoupons() async {
    ...
    await Future.forEach(couponsQuery.docs, (QueryDocumentSnapshot<Map<String, dynamic>> document) {
     //debugPrint("$TAG get All Coupons ========> ${document.data()}");
      try {
        coupon.add(OfferModel.fromJson(document.data()));
      } catch (e) {
       //debugPrint('FireStoreUtils.getAllCoupons Parse error $e');
      }
    });
    return coupon;
  }
  ...
  Future<List<StoryModel>> getStory() async {
    ...
    await Future.forEach(storyQuery.docs, (QueryDocumentSnapshot<Map<String, dynamic>> document) {
      try {
       ...
       //debugPrint('FireStoreUtils.getStory Parse error $e');
      }
    });
    return story;
  }
  ...
  Future<List<ProductModel>> getVendorProductsTakeAWay(String vendorID) async {
    ...
    QuerySnapshot<Map<String, dynamic>> productsQuery = await firestore.collection(PRODUCTS).where('vendorID', isEqualTo: vendorID).where("takeawayOption", isEqualTo: true).where('publish', isEqualTo: true).get();
    await Future.forEach(productsQuery.docs, (QueryDocumentSnapshot<Map<String, dynamic>> document) {
      try {
        products.add(ProductModel.fromJson(document.data()));
   ...
   //debugPrint("=====IDDDDDD" + products.length.toString());
    return products;
  }
  ...
  // comment by nilesh added on 31-05-2023 to show all the products to customer
  Future<List<ProductModel>> getVendorAllProducts(String vendorID) async {
    List<ProductModel> products = [];
  ...
  Future<List<OfferModel>> getOfferByVendorID(String vendorID) async {
    ...
    QuerySnapshot<Map<String, dynamic>> bannerHomeQuery = await firestore.collection(COUPON).where("resturant_id", isEqualTo: vendorID).where("isEnabled", isEqualTo: true).where('expiresAt', isGreaterThanOrEqualTo: Timestamp.now()).get();

   //debugPrint("-------->${bannerHomeQuery.docs}");
    await Future.forEach(bannerHomeQuery.docs, (QueryDocumentSnapshot<Map<String, dynamic>> document) {
      try {
       //debugPrint("-------->");
       //debugPrint(document.data());
    ...
    await Future.forEach(vendorsQuery.docs, (QueryDocumentSnapshot<Map<String, dynamic>> document) {
     //debugPrint(document);
      try {
       ...
       //debugPrint('FireStoreUtils.getOrders Parse error ${document.id} $e');
      }
    });
    return vendorreview;
  }
  ...
  Future<OrderModel> placeOrder(OrderModel orderModel) async {
   ...
   //debugPrint("$TAG place Order order Model =======> ${orderModel.toJson()}");
    return orderModel;
  }
  ...
  Future<OrderModel> placeOrderWithTakeAWay(OrderModel orderModel) async {
    ...
    if (orderModel.id.isEmpty) {
      documentReference = firestore.collection(ORDERS).doc(UserPreference.getOrderId());
      orderModel.id = documentReference.id;
    } else {
  ...
  static Future<List<TopupTranHistoryModel>> getTopUpTransaction() async {
    ...
    await Future.forEach(documentReference.docs, (QueryDocumentSnapshot<Map<String, dynamic>> document) {
      try {
       ...
       //debugPrint('FireStoreUtils.getAllProducts Parse error $e');
      }
    });
  ...
  static Future topUpWalletAmount({String paymentMethod = "test", bool isTopup = true, required amount, required id, orderId = ""}) async {
   //debugPrint("this is te payment id");
   //debugPrint(id);
   //debugPrint(MyAppState.currentUser!.userID);
  ...
  static Future updateWalletAmount({required amount}) async {
    ...
    await firestore.collection(USERS).doc(userId).get().then((value) async {
      ...
      if (userDocument.data() != null && userDocument.exists) {
        try {
         //debugPrint(userDocument.data());
          User user = User.fromJson(userDocument.data()!);
          MyAppState.currentUser = user;
         //debugPrint("$TAG update Wallet Amount ======> ${user.lastName.toString()} ===== ${user.walletAmount}");
         //debugPrint("add ${user.lastName} + $amount");
          await firestore.collection(USERS).doc(userId).update({"wallet_amount": user.walletAmount + amount}).then((value) {
            //debugPrint("north"));
          });
         ...
         //debugPrint(MyAppState.currentUser);
        } catch (error) {
         //debugPrint(error);
          if (error.toString() == "Bad state: field does not exist within the DocumentSnapshotPlatform") {
           ...
           //debugPrint("went wrong!!");
            walletAmount = "ERROR";
          }
        }
       //debugPrint("data val");
       //debugPrint(walletAmount);
  ...
  static Future<dynamic> loginWithEmailAndPassword(String email, String password) async {
    try {
     //debugPrint('FireStoreUtils.loginWithEmailAndPassword');
  ...
  static Future<dynamic> firebaseSubmitPhoneNumberCode(String verificationID, String code, String phoneNumber, BuildContext context, {String firstName = 'Anonymous', String lastName = 'User', File? image}) async {
   ...
   //debugPrint("$TAG authCredential =====> $authCredential");
   //debugPrint("$TAG userCredential =====> $userCredential");
   //debugPrint("$TAG userCredential get user id =====> ${userCredential.user?.uid}");

    User? user = await getCurrentUser(userCredential.user?.uid ?? '');

   //debugPrint("$TAG get current user by id ==========> $user");
    ...
    if (user != null) {
      if (user.role == USER_ROLE_CUSTOMER) {
        ...
        return 'You already registered with this number as "${user.role}" please use another number';
      }
    } else if (user == null) {
     ...
     //debugPrint("$TAG firebase Submit Phone Number Code errorMessage ========> $errorMessage");
     //debugPrint("$TAG firebase Submit Phone Number Code user ========> $user");
  ...
  static firebaseSignUpWithEmailAndPassword(String emailAddress, String password, File? image, String firstName, String lastName, String mobile, BuildContext context) async {
    try {
  ...
  static createNewOrderId() async {
    // DG20230001
    String orderId = "", prefix = "DG", suffix = "0000";
    int year = DateTime.now().year;

    await firestore.collection(ORDERS).get().then(
      (QuerySnapshot<Map<String, dynamic>> value) {
       //debugPrint("$TAG value id ======> ${value.size}");
        if (value.docs.length > 0) {
          var selectedValue = value.docs.last.id;
         //debugPrint("$TAG selected Value ========> $selectedValue");
          List<String> oldOrderId = selectedValue.split("$prefix$year");
         //debugPrint("$TAG oldOrderId ======> $oldOrderId");
          int finalSuffix = int.parse(oldOrderId[1]) + 1;
         //debugPrint("$TAG if finalSuffix =====> $finalSuffix");
          if (finalSuffix.toString().length < 4) {
            String formattedNumber = finalSuffix.toString().padLeft(4, '0');
           //debugPrint("$TAG if if formattedNumber ========> $formattedNumber");
            orderId = "$prefix$year$formattedNumber";
          } else {
            orderId = "$prefix$year$finalSuffix";
          }
         //debugPrint("$TAG orderId ========> $orderId");

          UserPreference.setOrderId(orderId: orderId);
        } else {
          var finalSuffix = int.parse(suffix) + 1;
         //debugPrint("$TAG else finalSuffix =====> $finalSuffix");
          if (finalSuffix.toString().length < 4) {
            String formattedNumber = finalSuffix.toString().padLeft(4, '0');
           //debugPrint("$TAG else if formattedNumber ========> $formattedNumber");
            orderId = "$prefix$year$formattedNumber";
          } else {
            orderId = "$prefix$year$finalSuffix";
          }
