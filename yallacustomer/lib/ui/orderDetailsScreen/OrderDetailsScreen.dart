...
class _OrderDetailsScreenState extends State<OrderDetailsScreen> {
  ...
  static const String TAG = "_OrderDetailsScreenState";
              ...
              return SingleChildScrollView(
                child: Column(
                  ...
                  children: [
                    ...
                    Visibility(
                      visible: ((orderStatus == ORDER_STATUS_PLACED || orderStatus == ORDER_STATUS_ACCEPTED || orderStatus == ORDER_STATUS_DRIVER_PENDING || orderStatus == ORDER_STATUS_DRIVER_REJECTED) && !isTakeAway),
                      child: Card(
                        shape: RoundedRectangleBorder(
                          borderRadius: BorderRadius.circular(8.0),
                        ),
  ...
  Widget buildOrderSummaryCard(OrderModel orderModel) {
    ...
    var totalamount = widget.orderModel.deliveryCharge == null || widget.orderModel.deliveryCharge!.isEmpty ? total + taxAmount - discount - specialDiscountAmount : total + taxAmount + double.parse(widget.orderModel.deliveryCharge!) + tipValue - discount - specialDiscountAmount;
            ...
            children: [
                                ...
                                onTap: () async {
                                  ...
                                  if (productModel!.itemAttributes != null) {
                                    if (productModel!.itemAttributes!.variants!.where((element) => element.variantSku == variantIno!.variantSku).isNotEmpty) {
                                      if (int.parse(productModel!.itemAttributes!.variants!.where((element) => element.variantSku == variantIno!.variantSku).first.variantQuantity.toString()) >= widget.orderModel.products[index].quantity) {
                                        ...
                                        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
                                          content: Text("Food is added in cart"),
                                        ));
                                      } else {
                                        await hideProgress();
                                        // ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
              ...
              (widget.orderModel.notes != null && widget.orderModel.notes!.isNotEmpty)
                  ? ListTile(
                      ...
                      trailing: InkWell(
                        onTap: () {
                          showModalBottomSheet(isScrollControlled: true, isDismissible: true, context: context, backgroundColor: Colors.transparent, enableDrag: true, builder: (BuildContext context) => viewNotesheet(widget.orderModel.notes!));
                        },
                        child: Text(
                          "View".tr(),
  ...
  Future<List<int>> getTicket() async {
    ...
    bytes += generator.row([
      PosColumn(text: '${widget.orderModel.address.line1} ${widget.orderModel.address.line2}, ${widget.orderModel.address.city}, ${widget.orderModel.address.country}', width: 12, styles: const PosStyles(align: PosAlign.right, height: PosTextSize.size1, width: PosTextSize.size1, bold: true)),
    ]);
    bytes += generator.row([
    ...
    bytes += generator.row([
      PosColumn(text: widget.orderModel.takeAway == false ? 'Deliver to door'.tr() : 'Takeaway'.tr(), width: 12, styles: const PosStyles(align: PosAlign.right, height: PosTextSize.size1, width: PosTextSize.size1, bold: true)),
    ]);
    bytes += generator.row([
    ...
    bytes += generator.row([
      PosColumn(text: DateFormat('dd-MM-yyyy, HH:mm').format(DateTime.fromMicrosecondsSinceEpoch(widget.orderModel.createdAt.microsecondsSinceEpoch)).toString(), width: 12, styles: const PosStyles(align: PosAlign.right, height: PosTextSize.size1, width: PosTextSize.size1, bold: true)),
    ]);
    bytes += generator.hr();
...
//   ]);
    ...
    bytes += generator.row([
      ...
      PosColumn(
          text: ((widget.orderModel.taxModel == null) ? "0" : getTaxValue(widget.orderModel.taxModel, total - discount - double.parse(widget.orderModel.specialDiscount!['special_discount'].toString())).toString()),
          width: 3,
          styles: const PosStyles(
            align: PosAlign.right,
            height: PosTextSize.size1,
  ...
  getDirections() async {
    if (currentOrder != null) {
      if (currentOrder!.status == ORDER_STATUS_SHIPPED) {
        ...
        LatLng driverLatLng = LatLng(_driverModel!.location.latitude, _driverModel!.location.longitude);
        LatLng customerLatLng = LatLng(currentOrder!.author.shippingAddress.location.latitude, currentOrder!.author.shippingAddress.location.longitude);
        ...
        debugPrint("$TAG ORDER_STATUS_SHIPPED poly line points ============> ${result.points}");
        ...
        addPolyLine(polylineCoordinates, driverLatLng, customerLatLng);
  ...
  addPolyLine(List<LatLng> polylineCoordinates, LatLng driverLatLng, LatLng customerLatLng) {
    setState(() {
      ...
      debugPrint("$TAG polylineCoordinates =========> ${polylineCoordinates.toString()}");
      if(polylineCoordinates != null && polylineCoordinates.isNotEmpty) {
        ...
        updateCameraLocation(driverLatLng, customerLatLng, _mapController);
      }
    });
  }
            ...
            children: [
              Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                ...
                Stack(
                  ...
                  children: [
                    displayCircleImage(order.driver?.carPictureURL ?? placeholderImage, 80, true),
