...
class _HomeScreenState extends State<HomeScreen> {
  ...
  _getLocation() async {
    ...
    //debugPrint("$TAG getLocation =========> called");
  ...
  getLoc() async {
    //debugPrint("$TAG getLoc =========> called");
    bool _serviceEnabled;
    _serviceEnabled = await location.serviceEnabled();
    //debugPrint("$TAG _serviceEnabled =========> $_serviceEnabled");
    if (_serviceEnabled) {
      //debugPrint("$TAG if _serviceEnabled =========> $_serviceEnabled");
      ...
      //debugPrint("$TAG else _serviceEnabled =========> $_serviceEnabled");
      getTempLocation();
    }
  ...
  @override
  void initState() {
    //debugPrint("$TAG initState ========> home called");
    super.initState();
    getLoc();
    getBanner();
  }
  ...
  getBanner() async {
    await fireStoreUtils.getHomeTopBanner().then((List<BannerModel> value) {
      setState(() {
        bannerTopHome = value;
        //debugPrint("$TAG banner Top Home ======> ${bannerTopHome.length}");
    ...
    await FirebaseFirestore.instance
      ...
      setState(() {
        //debugPrint("$TAG story isEnabled ======> ${value.data()}");
                ...
                child: Container(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      // comment by nilesh new arrival section need to modify on 02-06-2023
                      // Column(
                      //   mainAxisAlignment: MainAxisAlignment.start,
                      //   children: [
                      //     buildTitleRow(
                      ...
                      //       onClick: () {
                      //         push(context, const ViewAllNewArrivalRestaurantScreen(),);
                      //       },
                      //     ),
                                                                    ...
                                                                    children: [
                                                                      ...
                                                                      Row(
                                                                        children: [
                                                                          ...
                                                                          Expanded(
                                                                            child:
                                                                                Text(vendorModel.location, maxLines: 1, style: TextStyle(fontFamily: "Poppinsm", color: isDarkMode(context) ? Colors.white : Colors.black.withOpacity(0.60))).tr(),
                                                                          ),
                                                                        ],
                                                                      ),
                                                                      const SizedBox(
  ...
  Widget buildBestDealPage(BannerModel categoriesModel) {
    return InkWell(
      ...
      //   }
      //
      //  if (categoriesModel.redirect_type == "product") {
      //    print('BannerModel: product');
      //
      ...
      //
      //     //debugPrint("$TAG product Model ========> ${productModel.toJson()}");
      //     //debugPrint("$TAG vendor Model ========> ${vendorModel?.toJson()}");
      //    push(
      //      context,
      //      NewVendorProductsScreen(vendorModel: vendorModel!),
      ...
      //   //   //debugPrint("$TAG vendor Model ========> ${categoriesModel.redirect_id.toString()}");
  ...
  Future<void> getData() async {
    ...
    fireStoreUtils.getRestaurantNearBy().whenComplete(() async {
      ...
      lstAllRestaurant!.listen((List<VendorModel> event) {
        vendors.clear();
        vendors.addAll(event);
        allstoreList.clear();
        allstoreList.addAll(event);
        productsFuture.then((List<ProductModel> value) {
          for (int a = 0; a < event.length; a++) {
  ...
  Future<void> getTempLocation() async {
    //debugPrint("$TAG getTempLocation ========> called");
    //debugPrint("$TAG MyAppState.selectedPosotion.longitude ========> ${MyAppState.selectedPosotion.longitude}");
    //debugPrint("$TAG MyAppState.selectedPosotion.latitude ========> ${MyAppState.selectedPosotion.latitude}");
    if (MyAppState.currentUser == null &&
        MyAppState.selectedPosotion.longitude != 0 &&
        MyAppState.selectedPosotion.latitude != 0) {
      List<Placemark> placemarks = await placemarkFromCoordinates(
    ...
    if (MyAppState.currentUser != null) {
      //debugPrint("$TAG getTempLocation currentUser != null ========> called");
      //debugPrint("$TAG MyAppState.selectedPosotion.longitude ========> ${MyAppState.selectedPosotion.longitude}");
      //debugPrint("$TAG MyAppState.selectedPosotion.latitude ========> ${MyAppState.selectedPosotion.latitude}");
      ...
      children: [
        StoryView(
            ...
            onStoryShow: (StoryItem s) {
              //debugPrint("$TAG story item duration =====> ${s.duration}");
