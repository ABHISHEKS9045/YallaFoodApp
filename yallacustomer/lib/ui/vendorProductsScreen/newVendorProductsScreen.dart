...
class _NewVendorProductsScreenState extends State<NewVendorProductsScreen> with SingleTickerProviderStateMixin {
  ...
  String TAG = "_NewVendorProductsScreenState";
  ...
  void getFoodType() async {
    ...
    await fireStoreUtils.getVendorAllProducts(widget.vendorModel.id).then((List<ProductModel> value) {
      productModel.clear();
      productModel.addAll(value);
      getVendorCategoryById();
      setState(() {});
  ...
  getVendorCategoryById() async {
    ...
    setState(() {
      tabController = TabController(length: vendorCategoryModel.length, vsync: this);
    });
  ...
  SliverAppBar buildAppBar() {
    return FAppBar(
      ...
      vendorCateoryModel: vendorCategoryModel,
      isOpen: isOpen,
      context: context,
  ...
  SliverList buildBody() {
    return SliverList(
      delegate: SliverChildBuilderDelegate(
        ...
        childCount: vendorCategoryModel.length,
      ),
    );
  }
  ...
  Widget buildCategoryItem(int index) {
    ...
    VendorCategoryModel category = vendorCategoryModel[index];
    return RectGetter(
      key: itemKeys[index],
      child: AutoScrollTag(
        key: ValueKey(index),
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        child: Row(children: [
          ...
          //      //debugPrint("cart pro copre  " + cartProducts.length.toString());
          //      //debugPrint(cartProducts.toString());
          //      //debugPrint("cart pro co " + productModel.quantity.toString());
  ...
  statusCheck() {
   //debugPrint("$TAG statusCheck ========>");
    final now = new DateTime.now();
    var day = DateFormat('EEEE', 'en_US').format(now);
    var date = DateFormat('dd-MM-yyyy').format(now);
    widget.vendorModel.workingHours.forEach((WorkingHoursModel element) {
     //debugPrint("$TAG element day ========> ${element.day.toString()}");
      if (day == element.day.toString() || "Satuarday".toLowerCase() == element.day.toString().toLowerCase()) {
       //debugPrint("$TAG element day ========> " + element.day.toString());
        if (element.timeslot!.isNotEmpty) {
          element.timeslot!.forEach((Timeslot element) {
           //debugPrint("$TAG element time slot from ========> " + element.from.toString());
           //debugPrint("$TAG element time slot to ========> " + element.to.toString());
            ...
            if (isCurrentDateInRange(start, end)) {
             //debugPrint("$TAG if isCurrentDateInRange =======> ");
              setState(() {
                isOpen = true;
               //debugPrint("$TAG isOpen =======> $isOpen");
              });
            }
          });
        }
  ...
  bool isCurrentDateInRange(DateTime startDate, DateTime endDate) {
   //debugPrint("$TAG isCurrentDateInRange startDate ============> $startDate");
   //debugPrint("$TAG isCurrentDateInRange endDate ============> $endDate");
    final currentDate = DateTime.now();
   //debugPrint("$TAG isCurrentDateInRange currentDate ============> $currentDate");
