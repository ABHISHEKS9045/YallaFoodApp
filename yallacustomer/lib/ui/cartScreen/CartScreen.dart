...
class _CartScreenState extends State<CartScreen> {
  ...
  void initState() {
    super.initState();
    FireStoreUtils.createNewOrderId();
        ...
        Container(
          ...
          child: Row(
            ...
            children: [
              ...
              GestureDetector(
                onTap: () {
                  showModalBottomSheet(isScrollControlled: true, isDismissible: true, context: context, backgroundColor: Colors.transparent, enableDrag: true, builder: (BuildContext context) => sheet());
                },
        ...
        Container(
          ...
          child: Row(
            ...
            children: [
              ...
              GestureDetector(
                onTap: () {
                  showModalBottomSheet(isScrollControlled: true, isDismissible: true, context: context, backgroundColor: Colors.transparent, enableDrag: true, builder: (BuildContext context) => noteSheet());
                },
                child: const Image(image: AssetImage("assets/images/add.png"), width: 40),
              )
            ],
                ...
                child: Row(
                  ...
                  children: [
                    ...
                    Text(
                      ...
                      style: TextStyle(fontFamily: "Poppinsm", color: isDarkMode(context) ? const Color(0xffFFFFFF) : const Color(0xff333333), fontSize: selctedOrderTypeValue == "Delivery" ? _font : 15),
                    ),
                  ],
                ),
              ),
  ...
  addtocard(CartProduct cartProduct, int qun) async {
    await cartDatabase.updateProduct(CartProduct(id: cartProduct.id, name: cartProduct.name, photo: cartProduct.photo, price: cartProduct.price, vendorID: cartProduct.vendorID, quantity: qun, category_id: cartProduct.category_id, discountPrice: cartProduct.discountPrice!));
  }

  removetocard(CartProduct cartProduct, int qun) async {
    debugPrint("$TAG remove from cart ======> $qun");
    if (qun >= 1) {
      await cartDatabase.updateProduct(CartProduct(id: cartProduct.id, category_id: cartProduct.category_id, name: cartProduct.name, photo: cartProduct.photo, price: cartProduct.price, vendorID: cartProduct.vendorID, quantity: qun, discountPrice: cartProduct.discountPrice));
    } else {
                                ...
                                for (int a = 0; a < snapshot.data!.length; a++) {
                                  ...
                                  if (vendorID == couponModel.restaurantId || couponModel.restaurantId == "") {
                                    if (txt.text.toString() == couponModel.offerCode!.toString()) {
                                      debugPrint("$TAG offer code matched");
                                      message = "Your coupon code or voucher has been redeemed successfully";
                                      ...
                                      debugPrint("$TAG offer code matched");
                                      message = "Invalid voucher or coupon code";
                                    }
                                  }
                                }
                              });
  ...
  placeOrder() async {
    ...
    OrderModel orderModel = OrderModel(
      ...
      tax: getTaxValue(taxModel, subTotal - discountVal! - specialDiscountAmount).toStringAsFixed(decimal),
    );
                ...
                children: [
                  Container(
                    ...
                    child: Text(
                      ...
                      style: TextStyle(
                        fontFamily: 'Poppinssb',
                        color: isDarkMode(context) ? Color(0XFFD5D5D5) : Color(0XFF2A2A2A),
                        fontSize: 16,
                      ),
                    ),
                  ),
