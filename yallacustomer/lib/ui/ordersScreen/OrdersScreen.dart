...
class _OrdersScreenState extends State<OrdersScreen> {
  static const String TAG = "_OrdersScreenState";
                ...
                if (!snapshot.hasData || (snapshot.data?.isEmpty ?? true)) {
                  ...
                  return ListView.builder(
                    ...
                    itemBuilder: (context, index) {
                      return buildOrderItem(snapshot.data![index]);
                    },
                  );
                }
              }),
  ...
  Widget buildOrderItem(OrderModel orderModel) {
    ...
    orderModel.products.forEach((element) {
      try {
        ...
        photo = element.photo;
        debugPrint("$TAG for each loop image =======> ${element.photo}");
      } catch (ex) {
        debugPrint("$TAG catch error =====> $ex");
        debugPrint("$TAG catch error string =====> ${ex.toString()}");
      }
    });
  ...
  Color getOrderStatusColor(BuildContext context, String status) {
    if (status.toLowerCase() == "Order Accepted".tr().toLowerCase()) {
      return Colors.green;
    } else if (status.toLowerCase() == "Order Rejected".tr().toLowerCase()) {
      return Colors.red;
    } else {
      return isDarkMode(context) ? Colors.grey.shade200 : Color(0XFF555353);
    }
  }
